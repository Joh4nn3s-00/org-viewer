...
HTML in webview
  ...
  → buildToc()             (generates collapsible TOC sidebar with token count badge)
...
| Doc Map discovery | Scans workspace for .org files, classifies by layer, counts tokens per file, sends to webview |
...
Each file shows its token count (cl100k estimate) right-aligned in monospace. Sections display subtotals with a single-line divider, and a grand total appears at the bottom with a double-weight divider (accounting-ledger style). A faint vertical guide line on the right edge of the file tree aligns the numbers. Token counting uses gpt-tokenizer, reading each file via =vscode.workspace.fs.readFile()= in parallel.

** TOC Sidebar (webview.ts)

The Table of Contents sidebar is collapsible. A =«= button in the header collapses the sidebar to a narrow 28px strip, shifting the preview content left with a smooth transition. Click =»= to expand back to the full 180px width. The collapsed state hides the TOC items and token count badge.
